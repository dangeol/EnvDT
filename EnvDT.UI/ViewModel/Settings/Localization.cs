...
namespace EnvDT.UI.Settings.Localization
{
    // Taken from the following source:
    // https://github.com/Jinjinov/wpf-localization-multiple-resource-resx-one-language/blob/master/WpfLocalizationWithMultipleResourceManagers.cs
    public class TranslationSource : INotifyPropertyChanged
    {
        ...
        public string this[string key]
        {
            get
            {
                ...
                if (_resourceManagerDictionary.ContainsKey(baseName))
                    translation = _resourceManagerDictionary[baseName].GetString(stringName, currentCulture);
                return translation ?? key;
            }
        }

        private CultureInfo currentCulture = new CultureInfo(Properties.Settings.Default.Language);
        ...
        {
            if (!_resourceManagerDictionary.ContainsKey(resourceManager.BaseName))
            {
